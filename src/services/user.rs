...
use crate::models::{
    ApiResponse,
    user::{
        Invoice, Service, Session, Ticket, User, UserInvoiceSummary, UserReferralCode,
        UserTicketSummary,
    },
};
...
impl UserApiService {
    ...
    pub async fn get_invoice_by_id(&self, invoice_id: &str) -> Result<ApiResponse<Invoice>, Error> {
        self.client
            .send_request(
                Method::GET,
                &format!("/users/@me/invoices/{invoice_id}"),
                None,
            )
            .await
    }
    ...
    pub async fn get_invoices(&self) -> Result<ApiResponse<Vec<UserInvoiceSummary>>, Error> {
        self.client
            .send_request(Method::GET, "/users/@me/invoices", None)
            .await
    }

    pub async fn get_sessions(&self) -> Result<ApiResponse<Vec<Session>>, Error> {
        self.client
            .send_request(Method::GET, "/users/@me/sessions", None)
            .await
    }

    pub async fn get_referral_code(&self) -> Result<ApiResponse<UserReferralCode>, Error> {
        self.client
            .send_request(Method::GET, "/users/@me/referral", None)
            .await
    }
}
